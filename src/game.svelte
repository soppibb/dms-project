<script>
	...
	let potentialChoices = ["img/p1.jpg", "img/p2.jpg", "img/p3.jpg", "img/p4.webp", "img/p5.avif", "img/p6.webp", "img/p7.jpg", "img/p8.webp" ]
	...
	function onRoundStart() {
		if (params.mode == "diminishing") {
			sampleTime -= 100;
		} else if (params.mode == "delaying") {
			delayTime += 100;
		}
		correctAnswer = potentialChoices[Math.floor(Math.random() * potentialChoices.length)];
	}
		...
		if (selected == false) {
			...
			if (params.gameFeatures) {
				if (nbCorrect + nbIncorrect >= params.gameLength) {
					...
					if (
						nbCorrect >=
						params.gameThresholds.excel * params.gameLength
					) {
						if (params.playSound) {
							setTimeout(
					...
					if (params.timedRedirect) {
						...
						params.mode =
							params.randomModes[
								Math.floor(
									Math.random() * params.randomModes.length,
								)
							];
						setTimeout(() => {
							reset();
	...
	function newOne() {
		...
		exerciseResult = "";
		volatileParams.readyScreen = true;
		if (params.playSound && volatileParams.readyScreen)
			synth.speak(readySound);
	}
	...
	function manageSample() {
		...
		} else if (params.mode == "diminishing") {
			showSample = true;
			showChoices = false;
			setTimeout(() => {
				showChoices = true;
	...
	{#if !selected}
		{#if !volatileParams.readyScreen && volatileParams.ready}
			<div align="center" use:manageSample class="sample">
				<Abstractimage source={correctAnswer} active={showSample}></Abstractimage>
			</div>
